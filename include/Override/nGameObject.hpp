...
class nGameObject : public Util::GameObject,
					public std::enable_shared_from_this<nGameObject> // 爲了讓Component可以指向nGameObject
{
public:
	explicit nGameObject(const std::string &baseName = "", const std::string &baseClass = "nGameObject") :
		m_Id(GenerateUniqueID()), m_Name(GenerateUniqueName(baseName)), m_Class(baseClass)
	{
	}
	...
	[[nodiscard]] virtual std::string GetClassName() const { return m_Class; }
...
protected:
	int m_Id;
	std::string m_Name;
	std::string m_Class = "nGameObject";
	bool m_Active = true;
