...
std::shared_ptr<SaveData> SceneManager::DownloadGameProgress() const {
	if(m_Data) return std::make_shared<SaveData>(*m_Data); // 深拷貝一份
...
{
	if (const auto &saveManager = SaveManager::GetInstance();
...
void SceneManager::InitializeNewGameData()
{
	...
	// 直接初始化 SaveData 結構
	if (m_Data)newData = std::make_shared<SaveData>(*m_Data);
	else  newData = std::make_shared<SaveData>();
...
void SceneManager::receiveEnemyDeathEvent() const {
	m_CurrentScene->GetSaveData()->gameProgress.killCount++;
}
