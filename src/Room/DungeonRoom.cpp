...
void DungeonRoom::Update() { Room::Update(); }
...
void DungeonRoom::CreateWall(int row, int col)
{
	...
	glm::vec2 pos = Tool::RoomGridToWorld({col, row}, m_RoomSpaceInfo.m_TileSize, m_RoomSpaceInfo.m_WorldCoord,
...
{
	m_RoomConnection.SetNeighbor(dir, neighbor);
	m_RoomConnection.SetConnection(dir, hasConnection);
}

std::weak_ptr<DungeonRoom> DungeonRoom::GetNeighborRoom(Direction dir) const
{
	return m_RoomConnection.GetNeighbor(dir);
}

bool DungeonRoom::HasConnectionToDirection(Direction dir) const { return m_RoomConnection.HasConnection(dir); }

std::vector<std::weak_ptr<DungeonRoom>> DungeonRoom::GetConnectedNeighbors() const
{
	return m_RoomConnection.GetConnectedNeighbors();
}

std::vector<Direction> DungeonRoom::GetConnectedDirections() const { return m_RoomConnection.GetConnectedDirections(); }
