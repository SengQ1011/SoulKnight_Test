...
#include "Override/reverseAnimation.hpp"
...
namespace Util {
reverseAnimation::reverseAnimation(const std::vector<std::string> &paths, bool play,
    ...
    for (const auto &path : paths) {
        m_Frames.push_back(std::make_shared<Util::reverseImage>(path));
    }
}

void reverseAnimation::SetCurrentFrame(std::size_t index) {
    m_Index = index;
...
void reverseAnimation::Draw(const Core::Matrices &data) {
    m_Frames[m_Index]->Draw(data);
    Update();
}

void reverseAnimation::Play() {
...
void reverseAnimation::Pause() {
    if (m_State == State::PLAY || m_State == State::COOLDOWN) {
        m_State = State::PAUSE;
    }
}

void reverseAnimation::Update() {
