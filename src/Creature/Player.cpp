...
Player::Player(const std::string& ImagePath, int maxHp, float speed, int aimRange, CollisionBox* radius , std::unique_ptr<Weapon>  initialWeapon,
			   int maxArmor, int maxEnergy, double criticalRate, int handBladeDamage, std::shared_ptr<Skill> skill)
	: Character(ImagePath, maxHp, speed, aimRange, radius, std::move(initialWeapon)), m_maxArmor(maxArmor), m_currentArmor(maxArmor),
