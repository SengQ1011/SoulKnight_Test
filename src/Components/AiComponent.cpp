...
AIComponent::AIComponent(const MonsterType MonsterType, const std::shared_ptr<IMoveStrategy>& moveStrategy,
				...
				m_attackStrategy(attackStrategies), m_utilityStrategy(utilityStrategies),
				m_monsterPoint(monsterPoint), m_enemyState(enemyState::IDLE){}
...
void AIComponent::Update() {
	...
	if (m_moveStrategy) m_moveStrategy->Update(m_context, deltaTime);
	if (m_utilityStrategy) m_utilityStrategy->Update(m_context);
}
...
void AIComponent::SetEnemyState(const enemyState state) const {
	...
	if(state == enemyState::IDLE) stateComp->SetState(State::STANDING);
	else if(state == enemyState::WANDERING)stateComp->SetState(State::MOVING);
	else if(state == enemyState::CHASING)stateComp->SetState(State::MOVING);
	else if(state == enemyState::READY_ATTACK)stateComp->SetState(State::ATTACK);
}
